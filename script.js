...
function updateClock(){
    ...
    var dname = now.getDay(),
        ...
        var values = [ dnum.pad(2),months[mo],yr, week[dname],hou.pad(2),min.pad(2),sec.pad(2),pe];
        
        for(var i=0; i<ids.length;i++){
            document.getElementById(ids[i]).firstChild.nodeValue = values[i];
        }

        for(let i=0; i<scheduledAlarmArr.length;i++){
            if(scheduledAlarmArr[i]==`${hou.pad(2)}:${min.pad(2)}:${sec.pad(2)} ${pe}`){
                console.log("Alarm ringing...");
                ring.load();
                ring.play();
                document.querySelector("#stoptheAlarm").style.visibility= "visible";
            }
        }
}
...
function setAlarm(){
    ...
    if(time.includes("setHour") || time.includes("setMinute") || time.includes("AM/PM")){
        ...
        alarmCount++;
        document.querySelector(".scheduledAlarm").innerHTML += `
        ...
        scheduledAlarmArr.push(alarmTime);
...
function deleteAlarm(click_id){
    ...
    var deleteIndex = scheduledAlarmArr.indexOf(document.querySelector("#span"+click_id).innerText);
    scheduledAlarmArr.splice(deleteIndex,1);
    element.remove();
}

function stoptheAlarm(){
    ring.pause();
    document.querySelector("#stoptheAlarm").style.visibility= "hidden";
}
